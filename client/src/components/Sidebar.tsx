                ...
                if(isMessage) {
                  ...
                  return (
                    <div key={msg.id} className={`flex items-center gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-[80%] px-4 p-2 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? 'bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none' : 'rounded-tl-none bg-gray-800 text-gray-100'}`}>
                        {msg.content}
                      </div>
                    ...
                    <div key={version.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                      ...
                      <div className='flex items-center justify-between'>
                        {project.current_version_index === version.id ? (
                          ...
                          <button onClick={() => handleRollback(version.id)} className='px-3 py-1 rounded-md text-xs bg-indigo-500 hover:bg-indigo-600 text-white'>Roll back to this version</button>
                        )}
                        <Link target='_blank' to={`/preview/${project.id}/${version.id}`}>
          ...
          <form onSubmit={hadleRevisions} className='m-3 relative'>
            <div className='flex items-center gap-2'>
              <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4} placeholder='Describe your website or request changes...' className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all' disabled={isGenerating} />
                ...
                <button disabled={isGenerating || !input.trim()} className='absolute right-2.5 bottom-2.5 bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white rounded-full transition-colors disabled:opacity-60'>
                  {isGenerating ? <Loader2Icon className='size-7 animate-spin text-white'/> 
